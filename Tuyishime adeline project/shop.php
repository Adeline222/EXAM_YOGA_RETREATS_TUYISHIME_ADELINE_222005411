...
<h1>Shop Form</h1>
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        $stmt = $connection->prepare("INSERT INTO shop (id,name,address, created_at) VALUES (?, ?, ?, ?)");
        $stmt->bind_param("isss", $id,$name,$address, $created_at);


        // Set parameters from POST data with validation (optional)
        $id = intval($_POST['Cus_Id']); // Ensure integer for ID
        $name = htmlspecialchars($_POST['Ft_Nm']); // Prevent XSS
        $address = htmlspecialchars($_POST['ctid']); // Prevent XSS
        $created_at = filter_var($_POST['dscp']); 
...
<head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <title>Detail information Of product_out</title>
    <style>
        table {
            width: 100%;
...
<body>
    <center><h2>Table of product_out</h2></center>
    <table border="5">
        <tr>
        ...
        // Prepare SQL query to retrieve shop.
        $sql = "SELECT * FROM shop";
        $result = $connection->query($sql);

        // Check if there are any shop
        if ($result->num_rows > 0) {
            ...
            while ($row = $result->fetch_assoc()) {
                ...
                echo "<tr>
                    ...
                    <td><a style='padding:4px' href='delete_shop.php?id=$cuid'>Delete</a></td> 
                    <td><a style='padding:4px' href='update_shop.php?id=$cuid'>Update</a></td> 
                </tr>";
            }
...
<footer>
  <center> 
    <b><h2>UR CBE BIT &copy, 2024 & reg, Designer by: @Tuyishime Adeline</h2></b>
  </center>
</footer>
</body>
</html>